...
class TrendmicroSpamSpider(scrapy.Spider):
    website_name = "trendmicro_spam"
    name = "trendmicro_spam_link"
    allowed_domains = ["www.trendmicro.com"]
    start_urls = ["https://www.trendmicro.com/vinfo/us/threat-encyclopedia/spam"]

    # 用于和页数拼接
    page_base_url = "https://www.trendmicro.com/vinfo/us/threat-encyclopedia/spam/page/"

    # 用于和文章链接拼接
    article_base_url = "https://www.trendmicro.com"
    ...
    # 自制日志记录器
    myLog = logging.getLogger("TrendmicroSpam")

    def __init__(self, *args, **kwargs):
        super(TrendmicroSpamSpider, self).__init__(*args, **kwargs)
    ...
    def get_page_num(self, response):
        ...
        # page_num xpath路径
        page_num = int(response.xpath('//div[@class="paginationContainer"]/ul/li[last()]/a/div/text()').get())
        ...
        links.extend([self.article_base_url + href for href in response.xpath(
            '//div[@class="paddingContainer"]/div[@class="ContainerListTitle1"]/a/@href').getall()])
