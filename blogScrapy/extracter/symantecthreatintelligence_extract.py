...
if __name__ == '__main__':
    ...
    for uuid in all_uuids:
        ...
        title = dom.xpath('//h1[@id="page-title"]/text()')
        ...
        date = dom.xpath('//header[@class="blog-post__header"]//time/text()')
        date = date[0] if date else ''
        # print('date:', date)

        div_soup = soup.find(name="div",
                             class_="blog-post__content")

        spans_soup = div_soup.find_all(name="span",
                                       recursive=False)

        main_soup = soup.new_tag("div")
        for s in spans_soup:
            main_soup.append(s)
