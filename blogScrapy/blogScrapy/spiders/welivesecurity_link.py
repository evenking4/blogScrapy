...
class WelivesecuritySpider(scrapy.Spider):
    website_name = "welivesecurity"
    name = "welivesecurity_link"
    allowed_domains = ["/www.welivesecurity.com"]
    start_urls = ["https://www.welivesecurity.com/en/"]

    # 用于和页数拼接
    page_base_url = "https://www.welivesecurity.com/en/?page="

    # 用于和文章链接拼接
    article_base_url = "https://www.welivesecurity.com/"
    ...
    # 自制日志记录器
    myLog = logging.getLogger("Welivesecurity")

    def __init__(self, *args, **kwargs):
        super(WelivesecuritySpider, self).__init__(*args, **kwargs)
    ...
    def get_page_num(self, response):
        ...
        links.extend([self.article_base_url + href for href in response.xpath(
            '//div[@class="article-list-card"]/a/@href').getall()])
