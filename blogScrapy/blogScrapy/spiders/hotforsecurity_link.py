...
class HotforsecuritySpider(scrapy.Spider):
    website_name = "hotforsecurity"
    name = "hotforsecurity_link"
    allowed_domains = ["www.bitdefender.com"]
    start_urls = ["https://www.bitdefender.com/en-us/blog/"]

    # 用于和页数拼接
    page_base_url = "https://www.bitdefender.com/nuxt/api/get/en/hotforsecurity/posts/?limit=4&page="

    # 用于和文章链接拼接
    article_base_url = "https://www.bitdefender.com/en-us"
    ...
    # 自制日志记录器
    myLog = logging.getLogger("Hotforsecurity")

    def __init__(self, *args, **kwargs):
        super(HotforsecuritySpider, self).__init__(*args, **kwargs)
    ...
    def get_article_links(self, response):
        ...
        # 文章链接的xpath的路径
        links.extend([self.article_base_url + post["url"] for post in response.json()["posts"]])
