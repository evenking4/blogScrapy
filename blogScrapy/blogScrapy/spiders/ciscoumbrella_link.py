...
class CiscoumbrellaLinkSpider(scrapy.Spider):
    website_name = "ciscoumbrella"
    name = "ciscoumbrella_link"
    allowed_domains = ["umbrella.cisco.com"]
    start_urls = ["https://umbrella.cisco.com/blog"]

    # 用于和页数拼接
    page_base_url = "https://umbrella.cisco.com/blog/page/"
    ...
    # 自制日志记录器
    myLog = logging.getLogger("Ciscoumbrella")

    def __init__(self, *args, **kwargs):
        super(CiscoumbrellaLinkSpider, self).__init__(*args, **kwargs)
    ...
    def get_page_num(self, response):
        ...
        # page_num xpath路径
        page_num = int(response.xpath('//div[@class="archive-pagination pagination"]/ul/li[last()-1]/a/text()').get())
        ...
        # 文章链接的xpath路径
        links.extend(response.xpath('//a[@class="entry-title-link"]/@href').getall())
