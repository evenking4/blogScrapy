...
class CsoonlineSpider(scrapy.Spider):
    ...
    # 自制日志记录器
    myLog = logging.getLogger("Csoonline")

    def __init__(self, *args, **kwargs):
        super(CsoonlineSpider, self).__init__(*args, **kwargs)
    ...
    def get_page_num(self, response):
        ...
        # page_num xpath路径
        page_num = int(response.xpath('//ul[@class="pagination__items"]/li[last()-1]/a/text()').get().replace(",", ""))
        ...
        # 对第一页的特殊处理
        yield Request(url=self.page_base_url + '1',
                      headers=self.headers,
                      dont_filter=True,
                      cb_kwargs={'xpath': '//a[@class="grid content-row-article"]/@href'},
        ...
        for i in range(2, page_num + 1):
            ...
            yield Request(url=url,
                          headers=self.headers,
                          dont_filter=True,
                          cb_kwargs={'xpath': '//div[@class="content-listing-various__container"]/div[@class="content-listing-various__row"]/a/@href'},
                          callback=self.get_article_links,
                          errback=self.err_parse)
    ...
    def get_article_links(self, response, **kwargs):
        ...
        article_xpath = kwargs['xpath']

        # 文章链接的xpath的路径
        links.extend(response.xpath(article_xpath).getall())
