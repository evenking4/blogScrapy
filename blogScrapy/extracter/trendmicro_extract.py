...
if __name__ == '__main__':
    ...
    for uuid in all_uuids:
        ...
        title = dom.xpath('//h1[@class="article-details__title"]/text() | //h1[@class="mb-2 mt-4 text-333333 text-32 font-bold"]/text()')
        ...
        # 有一篇文章的title里面居然还用<br>分割，为其特殊处理
        title = ' '.join(title)

        tags = dom.xpath('//div[@class="tag--list-tags"]/a/text() | //div[@class="flex flex-wrap gap-2 mobile:justify-center items-center"]/a/text()')
        # print('tags:', tags)

        date = dom.xpath('//time[@class="article-details__date"]/text() | //time/@datetime')
        date = date[0] if date else ''
        # print('date:', date)

        main_soup = soup.find(name=["div", "main"],
                              class_=["main--content",
                                      "PostBody_content__s5k0A max-w-[1040px] w-full mb-10"])
