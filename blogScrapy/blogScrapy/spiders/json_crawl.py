...
class JsonCrawlSpider(scrapy.Spider):
    name = "json_crawl"
    ...
    def __init__(self, target=None, *args, **kwargs):
        super(JsonCrawlSpider, self).__init__(*args, **kwargs)
        self.target = target
    ...
    def statistics_links(self, all_links):
        for root, dirs, files in os.walk(f"./json_data/{self.target}/", topdown=True):
    ...
    # 确认返回的html内容正确(TODO)
    def detection_res(self, response):
        return True
    ...
    def handle_res(self, response, **kwargs):
        if not self.detection_res(response):
            # 移交给错误处理函数
        ...
        html_item = JsonItem()
        html_item["uuid"] = uuid
        html_item["url"] = response.request.url
        html_item["website"] = self.target
        html_item["filename"] = "raw.json"
        html_item["data"] = response.text
